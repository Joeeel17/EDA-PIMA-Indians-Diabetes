...
# Data Understanding
df <- read.csv("D:/Portfolio/Diabetes/data/pima-indians-diabetes.csv")
dim(df)
str(df)
head(df)
...
# method rf is random forests imputation 
  # which is for classification and regression
...
# 2 = False case
str(test)
clusplot(test.d, fit$cluster, color=TRUE, shade=TRUE,
         labels=2, lines=0)
table(test.d$glucose, fit$cluster)
plot(test.d[c("glucose", "insulin")], col=fit$cluster)
test.d$outcome <- as.factor(test.d$outcome)
plot(test.d[c("glucose", "insulin")], col=test.d$outcome)
# By comparing the plots, can see that the clusters were 
 # clustered proerply since theyre same

plot(test.d[c("skin", "bmi")], col=fit$cluster)
plot(test.d[c("skin", "bmi")], col=test$outcome)
